...
class VariationalAutoencoder(object):
    ...
    def _create_network(self):
        ...
        print(' Mean and variantion: ',self.z_mean[0],self.z_log_sigma_sq[0])
        ...
        print("Reconstracted Mean: ",self.x_reconstr_mean[0])

    def _initialize_weights(self, n_hidden_recog_1, n_hidden_recog_2, n_hidden_recog_3, n_hidden_recog_4, n_hidden_recog_5, n_hidden_recog_6, n_hidden_gener_1, n_hidden_gener_2,
                             n_hidden_gener_3, n_hidden_gener_4, n_hidden_gener_5, n_hidden_gener_6,n_input, n_z):
        all_weights = dict()
        all_weights['weights_recog'] = {
            ...
            'h5': tf.Variable(xavier_init(n_hidden_recog_4, n_hidden_recog_5)),
            'h6': tf.Variable(xavier_init(n_hidden_recog_5, n_hidden_recog_6)),
            'out_mean': tf.Variable(xavier_init(n_hidden_recog_6, n_z)),
            'out_log_sigma': tf.Variable(xavier_init(n_hidden_recog_6, n_z))}
        ...
        all_weights['weights_gener'] = {
            ...
            'out_mean': tf.Variable(xavier_init(n_hidden_gener_6, n_input)),
            'out_log_sigma': tf.Variable(xavier_init(n_hidden_gener_6, n_input))}
    ...
    def _recognition_network(self, weights, biases):
        ...
        z_mean = tf.add(tf.matmul(layer_6, weights['out_mean']),
                        biases['out_mean'])
        z_log_sigma_sq = \
            tf.add(tf.matmul(layer_6, weights['out_log_sigma']),
    ...
    def _generator_network(self, weights, biases):
        ...
        layer_6 = self.transfer_fct(tf.add(tf.matmul(layer_5, weights['h6']),
                                           biases['b6']))
        x_reconstr_mean = \
            tf.nn.sigmoid(tf.add(tf.matmul(layer_6, weights['out_mean']),
                                 biases['out_mean']))
    ...
    def _create_loss_optimizer(self):
        ...
        reconstr_loss = \
            -tf.reduce_sum(self.x * tf.log(1e-10 + self.x_reconstr_mean)
                           ...
                           1)
        print('reconstr loss: ',reconstr_loss[0])
