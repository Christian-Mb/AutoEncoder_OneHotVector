...
from lib.utils import xavier_init, corrupt
...
class DenoisingAutoencoder(object):
    def __init__(self, network_architecture, learning_rate=1e-3, batch_size=100, epochs=100,
        ...
        self.x_corrupt = corrupt(self.x) * self.corrupt_prob + self.x * (1 - self.corrupt_prob)
    ...
    # x -> x_hat
    def reconstructor(self, x, corrupt_rate):
        x_hat = self.session.run(self.x_hat, feed_dict={self.x: x, self.corrupt_prob: corrupt_rate})
        return x_hat

    # z -> x
    def generator(self, z, corrupt_rate):
        x_hat = self.session.run(self.x_hat, feed_dict={self.z: z, self.corrupt_prob: corrupt_rate})
        return x_hat

    # x -> z
    def transformer(self, x, corrupt_rate):
        z = self.session.run(self.z, feed_dict={self.x: x, self.corrupt_prob: corrupt_rate})
        return z

    def partial_fit(self, X, corrupt_rate):
        """Train model based on mini-batch of input data.

        Return cost of mini-batch.
        """
        opt, cost = self.session.run((self.optimizer, self.cost),
                                     feed_dict={self.x: X, self.corrupt_prob: corrupt_rate})
        return opt, cost
